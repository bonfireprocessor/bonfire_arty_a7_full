...
proc create_hier_cell_uart1 { parentCell nameHier } {
  ...
  if { $parentCell eq "" || $nameHier eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2092 -severity "ERROR" "create_hier_cell_uart1() - Empty argument(s)!"}
     return
  }
...
proc create_hier_cell_pmod_jb_spi { parentCell nameHier } {
  ...
  if { $parentCell eq "" || $nameHier eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2092 -severity "ERROR" "create_hier_cell_pmod_jb_spi() - Empty argument(s)!"}
     return
  }
  ...
  # Create interface connections
  connect_bd_intf_net -intf_net Conn1 [get_bd_intf_pins jb] [get_bd_intf_pins pmod_bridge_0/Pmod_out]
...
proc create_root_design { parentCell } {
  ...
  if { [catch {set BootMem_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
   ...
   CONFIG.RamFileName {/home/thomas/development/bonfire/bonfire_arty_a7_full/compiled_code/ARTY_AXI_monitor.hex}\
   CONFIG.SIZE {16384} \
   CONFIG.mode {H} \
 ] $BootMem_0
  ...
  connect_bd_intf_net -intf_net pmod_jb_spi_jb [get_bd_intf_ports jb] [get_bd_intf_pins pmod_jb_spi/jb]
